...
export default class Transmitter {
  ...
  constructor() {
    (this.client = axios.create({
      baseURL: "http://0.0.0.0:10080/api",
      timeout: 180000,
      headers: {
  ...
  async sendScrapedData(payload: ScrapedData): Promise<apiResponse> {
    //TODO:例外処理を入れる(型にvoidを追加するとレスポンスに対する処理がうまくいかない)
    const res = await this.client.post(ApiURIs.COMIC, payload).then((res) => {
